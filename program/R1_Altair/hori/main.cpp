...
{
    // 移動速度、旋回速度->それぞれの回転速度
    motor_target[0] = ((sqrt(2) / 2) * Vx + (sqrt(2) / 2) * Vy + R_radius * omega) / (O_diameter * M_PI);
    motor_target[1] = ((sqrt(2) / 2) * Vx - (sqrt(2) / 2) * Vy + R_radius * omega) / (O_diameter * M_PI);
    motor_target[2] = (-(sqrt(2) / 2) * Vx - (sqrt(2) / 2) * Vy + R_radius * omega) / (O_diameter * M_PI);
    motor_target[3] = (-(sqrt(2) / 2) * Vx + (sqrt(2) / 2) * Vy + R_radius * omega) / (O_diameter * M_PI);
}
...
void RoboMas_turn(void)
{
    ...
    can_2.write(CANMessage(id_act, RoboMas_data, 1));
